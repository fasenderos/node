...
class Test extends AsyncResource {
  ...
  constructor(options) {
    ...
    const { concurrency, loc, only, shuffle, timeout, todo, signal } = options;
    ...
    switch (typeof shuffle) {
      case 'number':
        validateUint32(shuffle, 'options.shuffle', 1);
        this.shuffle = true;
        this.seed = shuffle;
        break;
      case 'boolean':
          this.shuffle = shuffle;
          this.seed = shuffle ? Date.now() : null;
        break;
      default:
        if (shuffle != null)
          throw new ERR_INVALID_ARG_TYPE('options.shuffle', ['boolean', 'number'], shuffle);
    }
  ...
  start() {
    ...
    if (!this.parent.hasConcurrency() || 
        !this.parent instanceof Suite && this.parent.shuffle) {
      const deferred = createDeferredPromise();
  ...
  async run() {
    ...
    try {
      ...
      if (this.shuffle && this.pendingSubtests.length) {
        shuffle(this.pendingSubtests, this.seed);
        await this.processPendingSubtests();
      }
  ...
  constructor(options) {
    ...
    try {
      ...
      this.buildSuite = SafePromisePrototypeFinally(
        ...
        () => {
          if (this.shuffle && this.subtests.length > 1) {
            shuffleArray(this.subtests, this.seed);
          }
